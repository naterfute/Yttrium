...
def migrateDb(
  ...
  if not engineType['database'] in ['sqlite', 'mysql', 'mariadb']:
    alembic_cfg.set_main_option(
      'sqlalchemy.url',
      f'{engineType["database"]}://{username}:{password}@{host}:{port}/{database}',
    )
  elif engineType['database'] not in ['postgres', 'sqlite']:
    alembic_cfg.set_main_option(
      'sqlalchemy.url',
      f'{engineType["database"]}+pymysql://{username}:{password}@{host}:{port}/{database}',
    )
  elif engineType['database'] in ['sqlite']:
    alembic_cfg.set_main_option(
      'sqlalchemy.url', f'{engineType["database"]}:///{database}.sqlite'
    )
  else:
